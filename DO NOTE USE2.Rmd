...
=======
>>>>>>> 105ef90745c3db6ab0f955b537df872ee07e7730
# The Process
...
The dataset that was used for this analysis contains the following variables: 
...
4. Average monthly hours - From `r min(ProjectData[,4])` to `r max(ProjectData[,4])`
...
1. What causes employees to leave the company? 

2. What are the most important factors that differ between those who stay and those who leave? (segmentation)

3. How do we use this to our advantage to ensure that people stay as long as possible? 

4. Are our best employees overworked?
...
Clearly the survey asked many redundant questions (can you think some reasons why?), so we may be able to actually "group" these 10 attitude questions into only a few "key factors". This not only will simplify the data, but will also greatly facilitate our understanding of the employees.
...
1. What do we learn from these plots? 
...
The first couple of factors mean



The factors that should be chosen for this data is based on: a) statistical arguments, b) on interpretation arguments, c) on business arguments 
...
So according to the data, some of the people can be described g the new derived variables (factor scores)
...
do.call(iplot.grid, lapply(variables_to_plot, function(n){
  iplot.hist(ProjectData_segment[, n], breaks=2, xlab = paste("Variable", n))
}))
```
...
iplot.hist(Pairwise_Distances, breaks=2)
```
